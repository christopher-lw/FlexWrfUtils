...
class AgeclassArgs(FlexWrfEnum):
    START_DELIMITER = (
        start_delimiter
...
class ReceptorArgs(FlexWrfEnum):
    START_DELIMITER = (
        start_delimiter
...
class ReleasesArgs(FlexWrfEnum):
...
class FlexWrfOption:
    def __init__(self, _file_content: List[str], _Argument_Enum: FlexWrfEnum):
        self._file_content = _file_content
        self._Argument_Enum = _Argument_Enum
        self._number_of_lines = len(self._Argument_Enum)
        # self._argument_values = self._read_content()

    @property
    def argument_order(self) -> Dict[FlexWrfEnum, int]:
        argument_order = dict(
            zip(self._Argument_Enum, np.arange(self._number_of_lines))
        )
    ...
    def update(self, argument: Union[str, FlexWrfEnum], value: Any):
        ...
        line_index = self.argument_order[argument]
